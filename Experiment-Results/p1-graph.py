import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
import numpy as np
import glob
import os

def extract_params_from_filename(filename):
    # Extract TTFW and SS from filename
    basename = os.path.basename(filename)
    # remove .csv extension
    basename = basename[:-4]
    params = basename.split('-')[1:]
    TTFW = float(params[1])
    SS = int(params[3])
    return TTFW, SS

# Get list of all CSV files
directory = './HW A-Star P1'
files = glob.glob(os.path.join(directory, '*.csv'))

data = []

# Load each file and calculate average win/fail value
for file in files:
    TTFW, SS = extract_params_from_filename(file)
    df = pd.read_csv(file, skiprows=2)
    df['win/fail'] = df['win/fail'].map({True: 1, False: 0})
    avg_win_fail = round(df['win/fail'].mean(), 3)
    data.append([TTFW, SS, avg_win_fail])

# Create DataFrame
df = pd.DataFrame(data, columns=['TTFW', 'SS', 'avg_win_fail'])

# Pivot DataFrame to create matrix
pivot_df = df.pivot(index='TTFW',columns='SS',values='avg_win_fail')

# Plot heatmap
plt.figure(figsize=(10, 8))
sns.heatmap(pivot_df, annot=True, fmt='g', cmap='viridis')
plt.title('Average Win/Fail Heatmap')

# Set x and y labels
plt.xlabel('Search Steps (SS)')
plt.ylabel('Time to Finish Weight (TTFW)')

plt.gca().invert_yaxis()

# Save heatmap
plt.savefig('p1-heatmap.png')